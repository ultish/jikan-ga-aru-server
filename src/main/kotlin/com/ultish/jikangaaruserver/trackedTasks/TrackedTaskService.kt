...
class TrackedTaskService {
    ...
    fun trackedTask(
        ...
        val builder = BooleanBuilder().and(QETrackedTask.eTrackedTask.id.eq(trackedTaskId))
    ...
    fun trackedTasks(
        ...
        return dgsQuery(dfe) {
            ...
            trackedDayId?.let {
                builder.and(
                    QETrackedTask.eTrackedTask.trackedDayId.eq(
                        trackedDayId
                    )
                )
            }
    ...
    val loadForTrackedTaskBatchLoader =
        MappedBatchLoaderWithContext<String, TrackedDay> { trackedTaskIds, environment ->
...
//      }
    ...
    val loadForChargeCodes = MappedBatchLoaderWithContext<String, List<ChargeCode>> { trackedTaskIds, env ->
        CompletableFuture.supplyAsync {
            ...
            val chargeCodeIds = trackedTaskToChargeCodesMap.values.flatten()
                .distinct()
